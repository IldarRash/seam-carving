...
fun negative(inputFile: String, outPut: String) {
    fun negative(color : Int): Int {
        val c = Color(color)
        return Color(255 - c.red, 255 - c.green, 255 - c.blue).rgb

    }
    val bufferedImage = ImageIO.read(File(inputFile))
    for (i in 0 until bufferedImage.width) {
        for (j in 0 until bufferedImage.height) {
            bufferedImage.setRGB(i, j, negative(bufferedImage.getRGB(i, j)))
        }
    }
